	...
	if (!$con)
	  {
		...
		$result = mysql_query("SELECT * FROM `newslist` WHERE id<'7'", $con);
        $dataArr = array();
...
function insert(){
  ...
  if (!$con)
  {
    die('Could not connect: ' . mysql_error());
      ...
      $sql = "INSERT INTO `newslist` (`url`, `pic`, `title`,`content`,`topic`,`time`,`id`) VALUES ( '$url', '$pic', '$title', '$content', '$topic', '$time', '$id')";
